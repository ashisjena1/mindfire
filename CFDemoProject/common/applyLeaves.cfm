...
<cfif structKeyExists(SESSION,"setLoggedInUser")>
	...
	<cfif #UCase(SESSION.setLoggedInUser.empRole)# EQ "HR">
		...
		<cfelse>
			...
			<cfset VARIABLES.applyLeaveErrorMessage=APPLICATION.personalService.checkApplyLeaveDetails(SESSION.setLoggedInUser.empId)/>
			<cfif VARIABLES.applyLeaveErrorMessage.fromDate EQ ""  and VARIABLES.applyLeaveErrorMessage.toDate EQ "" and
					...
					<cfset VARIABLES.isLeaveApplied = APPLICATION.personalService.addLeave(SESSION.setLoggedInUser.empId,FORM.fromDate,FORM.toDate,FORM.totalDays,FORM.reason,FORM.year)/>
			</cfif>
		</cfif>
